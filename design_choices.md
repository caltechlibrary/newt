...
Second question, "Why Pandoc?". At the time of the first prototype it made sense. It was there, it was "off the shelf" and I figured out how to create a POST to it that let me turn my JSON into a web page. When trying to generalize the Newt data router it proved very problematic. So the experimental outcome was while it helped me bootstrap Newt it wasn't worth keeping it in the mix. Mustache templates was my othere choice but there wasn't a riable "off the shelf" service I wanted to work with. Fortunately there was a good implementation of Mustache templates for the Go programming language. I wrote a pipeline friendly template engine for Newt. You send it a JSON object and based on configuration those objects are transformed by your Mustache template. The nice thing about this is the template engine reads in the templates at startup and also makes them available via URL paths. This makes it easier to know what's happening in your YAML file should you need to change things up.

The third question, "Why YAML for configuration?". Personally I'm ambivilant about YAML. It is sort of boring. But I think that is is strength. YAML has become umbiquious for describing configuration and simple data structures. It easily converts to JSON. It's declarative. Newt uses YAML largely because it's well known and is specifically known by my colleagues at Caltech Library. Why invent a new language when I can use one that is already known?

Final question is "How do I handle file uploads?". The short answer is Newt doesn't. In theory you could write a service to handle it and use it in a router pipeline but Newt doesn't provide one. Long answer: Technically if you clever you could transform and encode the data browser side to make it easy to store in Postgres via PostgREST. I don't recommend that. It is more trouble than it is worth. 
...
[4]: The group behind PostgREST is also developing an Postgres embedable Mustache engine, that may make sense too.
